...
# Define the backbone network for CnnNet, such as classification and detection
...
class CnnNet(Model):
    ...
    # Get the latency form predictor or benchmark
    def get_latency(self, latency_func):
        return latency_func(self)

    # Get the Conv layers of the network
    def get_layers(self):
        n = 0
        for block in self.block_list:
            n += block.get_layers()
    ...
    # Get the parameters of the network for latency prediction
    ...
    # Get the madnas score of the network, which does not need forward on GPU and has very fast speed
    ...
    # Get the deepmad score of the network, which does not need forward on GPU and has very fast speed
    ...
    # Get efficient_score of the network
    ...
    # Get the number of max feature map for MCU
